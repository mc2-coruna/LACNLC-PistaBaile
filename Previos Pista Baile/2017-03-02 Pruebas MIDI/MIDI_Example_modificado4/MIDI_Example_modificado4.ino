...
void TileOn(unsigned int matrix){
  ...
  for (int i=0; i<16; i++)
  {
    ...
    if (1 & (matrix>>i))
    {
      ...
      talkMIDI(0xB0 | 0, 0x07, 40);  // 0xB0 is channel message, set channel volume bajo (40)
      //noteOn(i, tileNote [i], 120);  // usa el canal i
      noteOn(0, tileNote [i], 120);  // usa el canal i
    }
  }
...
void TileStep(unsigned int matrix){
  ...
  for (int i=0; i<16; i++)
  {
    ...
    if (miliSeconds > tileOffTime [i] + timeNoteOff)
    {
      ...
      talkMIDI(0xB0, 0x78,0);        // Set channel all sound off 0xB0
      tileSteped [i] = false;
    }
    ...
    else if (!tileSteped [i])
    {
      ...
      if (matrix & (1<<i))
      {
        ...
        talkMIDI(0xB0, 0x07, 120);     //0xB0 is channel message, set channel volume ALTO (120)
        //noteOn(i, tileNote [i], 127);  // usa el canal i
        noteOn(0, tileNote [i], 127);  // usa el canal i
        tileSteped [i] = true;
